...
func TestNoScimClient(t *testing.T) {
	...
	_, err := p.GetUsersForGroup(context.Background(), &groupRequest)

	assert.Error(t, err)
	assert.ErrorIs(t, err, plugin.ErrNoScimClient)

	userRequest := idmangv1.GetGroupsForUserRequest{}
	_, err = p.GetGroupsForUser(context.Background(), &userRequest)
...
func TestGetUsersForGroup(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			responseMsg, err := p.GetUsersForGroup(context.Background(), &request)
